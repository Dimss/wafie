...
func main() {
	...
	links, err := attachToMultipleInterfaces(objs, interfaces)
	...
	for range ticker.C {
		...
		//_, err := formatMapContents("src", objs.IpMap)
		//if err != nil {
...
func formatMapContents(name string, m *ebpf.Map) (string, error) {
	var (
		//sb  strings.Builder
		key netip.Addr
		val uint32
	...
	for iter.Next(&key, &val) {
		...
		packetCount := val
		log.Printf("[name: %s ip: %s packets: %d]\n", name, sourceIP, packetCount)

	}
	return "", nil
}

func attachToMultipleInterfaces(obj bpfObjects, interfaceNames []string) ([]link.Link, error) {
	var links []link.Link
