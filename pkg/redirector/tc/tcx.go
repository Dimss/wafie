...
func attachTCProgram(i *net.Interface, objs bpfObjects) (link.Link, error) {
	//log.Println("attaching XDP to: "+i.Name, " index: ", i.Index)
	//return link.AttachXDP(
	//	link.XDPOptions{
	//		Interface: i.Index,
	//		Program:   objs.RedirectToProxyXdp,
	//	},
	//)
	log.Println("attaching TC to: "+i.Name, " index: ", i.Index)
	return link.AttachTCX(
		link.TCXOptions{
			Interface: i.Index,
			Program:   objs.RedirectToProxy,
...
func readIPMap(ipMap *ebpf.Map) {
	...
	for iter.Next(&key, &value) {
		...
		//wpDstIp := net.ParseIP("10.244.0.31")
		//if srcIP.Equal(wpSrcIp) && dstIP.Equal(wpDstIp) {
		//if dstIP.Equal(wpSrcIp) {
